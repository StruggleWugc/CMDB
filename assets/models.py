...
class Assets(models.Model):
    ...
    assets_type = models.CharField(choices=assets_type_choice, max_length=64, default='server', verbose_name='资产类型')
    ...
    status = models.SmallIntegerField(choices=assets_status, default=0, verbose_name='设备状态')
    ...
    admin = models.ForeignKey(User, null=True, verbose_name='资产管理员', related_name='admin', on_delete=models.SET_NULL)
    idc = models.ForeignKey('IDC', null=True, blank=True, verbose_name='所在机房', on_delete=models.SET_DEFAULT)
    ...
    def __str__(self):
        return '<%s> %s' % (self.get_assets_type_display(), self.name)
...
class Server(models.Model):
    ...
    create_by = models.CharField(choices=created_by_choice, max_length=64, default='auto', verbose_name='添加方式')
...
class NetworkDevice(models.Model):
    ...
    firmware = models.CharField(max_length=128, blank=True, null=True, verbose_name='设备固件型号')
...
class Software(models.Model):
    ...
    version = models.CharField(max_length=128, unique=True, help_text='例如： RedHat release 7 (Final)',
...
class Manufacturer(models.Model):
    ...
    telephone = models.CharField('客服电话', max_length=32, blank=True, null=True)
...
class Contract(models.Model):
    ...
    detail = models.TextField('合同详情', null=True, blank=True)
    star_day = models.DateField('开始日期', blank=True, null=True)
    end_day = models.DateField('结束日期', blank=True, null=True)
...
class CPU(models.Model):
    ...
    cpu_core_count = models.PositiveSmallIntegerField('CPU核心数', default=1)
...
class RAM(models.Model):
    ...
    sn = models.CharField('SN号', max_length=64, blank=True, null=True)
...
class Disk(models.Model):
    ...
    interface_type = models.CharField('接口类型', choices=disk_interface_choice, max_length=16, default='unknown')
...
class NIC(models.Model):
    ...
    mac = models.CharField('MAC地址', max_length=64)  # 虚拟机可能会出现相同的Mac地址
    ...
    class Meta:
        ...
        unique_together = ('asset', 'model', 'mac')  # 资产、型号和Mac必须联合唯一，防止虚拟机中的特殊情况发生


class EventLog(models.Model):
    """
    日志
    在关联对象呗删除时，不能一并删除，要保留日志
    隐藏，on_delete=models.SET_NULL
    """
    ...
    component = models.CharField('事件子项', max_length=512, blank=True, null=True)
    ...
    memo = models.CharField('备注', max_length=512, blank=True, null=True)
...
class NewAssetApprovalZone(models.Model):
    ...
    data = models.TextField('资产数据',unique=True)     #此字段必填
    ...
    class Meta:
        verbose_name = '新上线待批资产'
        verbose_name_plural = '新上线待批资产'
        ordering = ['-c_time']
